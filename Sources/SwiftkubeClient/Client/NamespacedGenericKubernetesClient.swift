...
public extension NamespacedGenericKubernetesClient where Resource: ReadableResource {
	...
	///
	/// ```swift
	/// let task = try client.pods.watch(in: .default)
...
public extension NamespacedGenericKubernetesClient where Resource: MetadataHavingResource {
	func deleteLabel(in namespace: NamespaceSelector? = nil, name: String, labelName: String) async throws -> Resource {
		return try await super.deleteLabel(in: namespace ?? .namespace(config.namespace), name: name, label: labelName)
	}
	func setLabels(in namespace: NamespaceSelector? = nil, name: String, labelName: String, value: [String:String]) async throws -> Resource {
		return try await super.setLabels(in: namespace ?? .namespace(config.namespace), name: name, value: value)
	}
}
